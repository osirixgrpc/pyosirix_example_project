...
def create_favicon(input_path, output_path, size=(16, 16)):
    try:
        ...
        # Remove unwanted space around the image (Powerpoint...).
        rgba = np.array(img)
        r, c = np.where(rgba[..., -1] > 0)
        bbox = [min(r), min(c), max(r), max(c)]
        rgba_crop = rgba[bbox[0]:bbox[2], bbox[1]:bbox[3], :]
        img = Image.fromarray(rgba_crop)
